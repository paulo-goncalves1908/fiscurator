...
function createSecondaryTable(conn, table) {
  return new Promise((resolve, reject) => {
    var sql = `CREATE TABLE IF NOT EXISTS CURATOR.${table}_NPS (iduser VARCHAR(1000) NOT NULL,conversationID VARCHAR(1000) NOT NULL,channel VARCHAR(1000),starttime timestamp,timeInterval INTEGER NOT NULL, feedback SMALLINT, transfered BOOLEAN, relevance BOOLEAN,newuser BOOLEAN NOT NULL,PRIMARY KEY(conversationID));`;
    try {
      conn.query(sql, (err) => {
        if (err) {
          if (err.sqlcode === 4136 || err.sqlcode === -601) {
...
function createQuaternaryTable(conn, table) {
  return new Promise((resolve, reject) => {
    var sql = `CREATE TABLE IF NOT EXISTS CURATOR.${table}_NPS (conversationID VARCHAR(1000) NOT NULL,envVariableName VARCHAR(1000) NOT NULL,envVariableValue VARCHAR(1000),envVariableType VARCHAR(1000));`;
    try {
      conn.query(sql, (err) => {
        if (err) {
          if (err.sqlcode === 4136 || err.sqlcode === -601) {
...
function createQuinaryTable(conn, table) {
  return new Promise((resolve, reject) => {
    var sql = `CREATE TABLE IF NOT EXISTS CURATOR.${table}_NPS (conversationID VARCHAR(1000) NOT NULL, originNode VARCHAR(1000), destineNode VARCHAR(1000));`;
    try {
      conn.query(sql, (err) => {
        if (err) {
          if (err.sqlcode === 4136 || err.sqlcode === -601) {
